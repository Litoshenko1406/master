...
import { User, Bot, Loader2, FileCode } from "lucide-react";
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const args = tool.args as { path?: string; command?: string; new_path?: string };
                            const rawPath = args?.command === "rename" ? (args?.new_path || args?.path) : args?.path;
                            const fileName = rawPath ? rawPath.split("/").filter(Boolean).pop() : null;
                            const displayName = fileName || tool.toolName;
                            ...
                            const actionLabel = args?.command ? (ACTION_LABELS[args.command] ?? null) : null;
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 bg-neutral-50 rounded-md text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <FileCode className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                                    <span className="font-medium text-neutral-800 font-mono">{displayName}</span>
                                    {actionLabel && (
                                      <span className="text-neutral-400 ml-0.5">· {actionLabel}</span>
                                    )}
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-500 shrink-0" />
                                    <span className="text-neutral-600 font-mono">{displayName}</span>
                                  </>
                                )}
                              </div>
                            );
